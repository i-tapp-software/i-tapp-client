...
export type FilterProps = {
  duration: { id: string; time: string; checked: boolean }[];
  location: string;
  field: { id: string; industry: string; checked: boolean }[];
};
...
export default function FilterCompanies({
  ...
  const handleMonthChange = (id: string, checked: boolean) => {
    ...
    >
      <div className=" pb-6 border-b  flex justify-between  ">
        ...
        <p className="  text-primary underline underline-offset-3 text-sm lg:text-base">
          <Link href="" onClick={() => setFilter(student.filters)}>
            Reset All
          </Link>
        </p>
      </div>
...
export const handleFilter = (
  ...
  if (filteredField.length) {
    filteredCompanies = filteredCompanies?.filter((company) =>
      filteredField.includes(company.industry)
    );
  }
